...
`r BiocStyle::Biocpkg("TreeSummarizedExperiment")` data container. Much of the
...
The `r BiocStyle::Biocpkg("tidySingleCellExperiment")` package provides handy
functions for `(Tree)SE` manipulation.
...
data(hitchip1006)
tse <- hitchip1006
tse_tibble <- tse |> arrange(subject, time)
```
